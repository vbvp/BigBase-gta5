...
namespace Big
{
	...
	void MainScript::Initialize()
	{
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Home", SubmenuHome, [](RegularSubmenu* sub)
			{
				...
				sub->AddOption<SubOption>("Player", nullptr, SubMenuPlayerOptions);
				sub->AddOption<SubOption>("Network", nullptr, SubmenuPlayerList);
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Self", SubMenuPlayerOptions, [](RegularSubmenu* sub)
		{
				sub->AddOption<BoolOption<std::atomic_bool>>("God Mode", nullptr, &g_Settings.self_god_mode, BoolDisplay::OnOff);
				sub->AddOption<NumberOption<std::int32_t>>("Set Health", nullptr, &g_Settings.self_set_health, 0, 10000000);
				sub->AddOption<NumberOption<std::int32_t>>("Set Max Health", nullptr, &g_Settings.self_set_max_health, 0, 10000000);
				sub->AddOption<NumberOption<float>>("Health Regen Multiplier", nullptr, &g_Settings.self_set_health_regen_x, 0.f, 100.f, 0.1f, 1);
				sub->AddOption<BoolOption<std::atomic_bool>>("Cops Never Wanted", nullptr, &g_Settings.self_never_wanted, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Police Blind Eye", nullptr, &g_Settings.self_cops_blind_eye, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Gangs Blind Eye", nullptr, &g_Settings.self_gangs_blind_eye, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Everyone Ignore Player", nullptr, &g_Settings.self_ignore_player, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Ingore Shocking Events", nullptr, &g_Settings.self_ignore_shocking_events, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Max Run Speed", nullptr, &g_Settings.self_max_run_speed, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Max Swim Speed", nullptr, &g_Settings.self_max_swim_speed, BoolDisplay::OnOff);
				sub->AddOption<NumberOption<float>>("Run Multiplier", nullptr, &g_Settings.self_run_speed_x, 0.f, 10.f, 0.1f, 1);
				sub->AddOption<NumberOption<float>>("Swim Multiplier", nullptr, &g_Settings.self_swim_speed_x, 0.f, 10.f, 0.1f, 1);
				sub->AddOption<BoolOption<std::atomic_bool>>("Super Jump", nullptr, &g_Settings.self_super_jump, BoolDisplay::OnOff);
				sub->AddOption<BoolOption<std::atomic_bool>>("Invisible", nullptr, &g_Settings.self_invisible, BoolDisplay::OnOff);
